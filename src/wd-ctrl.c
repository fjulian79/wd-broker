/*
 * wd-ctrl – Command-line tool for interacting with the wd-broker daemon.
 *
 * Provides root-level control for inspecting broker state and managing clients
 * via a structured UNIX socket protocol. Designed for use on embedded Linux
 * systems and intended to be simple, clear, and safe by default.
 *
